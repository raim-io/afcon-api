...
test.group('Groups store', (group) => {
  ...
  test('should create a new group', async ({ client, route, assert }) => {
  ...
  test('should return 422 if the name filed in the request body has invalid length for creating a group', async ({
    client,
    route,
  }) => {
    const groupName = 'Group NameTooLongNameTooLongNameTooLongNameTooLongNameTooLongNameTooLong'
    const response = await client.post(route('GroupsController.store')).form({
      name: groupName,
    })
  ...
  test('should return 422 if the name field in the request body is not unique for creating a group', async ({
    client,
    route,
  }) => {
